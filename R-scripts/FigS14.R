...
##### OPTIMISTIC SCENARIOS: UNPACK sens.double.zip
...
layout(matrix(1:10, 2, 5, byrow = TRUE), respect = F)
...
##### COMPARE WITH COMBINED STRATEGY UNDER BASE-CASE UPTAKE
setwd("../combi.base")
...
plot.new();
plot(x=c(-5, 75), y=c(0, 0.05), type="n", xlab="Years since start vaccination program", ylab="HPV16 prevalence, penile")
for (i in 1:360) lines(y.model15[i,] ~ time, col="grey")
lines(apply(y.model15 * as.vector(aic.weights), 2, sum) ~ time, col=2)
lines(apply(y.model15, 2, function(x) {quantile(x,0.05)}) ~ time, col=2, lty=3)
lines(apply(y.model15, 2, function(x) {quantile(x,0.95)}) ~ time, col=2, lty=3)
box()
title("40% boys at 12y + all MSM")
...
plot.new();
plot(x=c(-5, 75), y=c(0, 0.15), type="n", xlab="Years since start vaccination program", ylab="HPV16 prevalence, anal")
for (i in 1:360) lines(y.model25[i,] ~ time, col="grey")
lines(apply(y.model25 * as.vector(aic.weights), 2, sum) ~ time, col=2)
lines(apply(y.model25, 2, function(x) {quantile(x,0.05)}) ~ time, col=2, lty=3)
lines(apply(y.model25, 2, function(x) {quantile(x,0.95)}) ~ time, col=2, lty=3)
box()
title("40% boys at 12y + all MSM")
