...
plot.new(); boxplot(t(red.matrix[table.order,,3,1]), xaxt="n",
 ylab="HPV16 prevalence reduction, penile", col=cols[modcols]); box()
text(x=c(0.5,5,12,18), y=0.039, c("SIS","systemic\nimmunity","local\nimmunity", "latency"), cex=0.9, xpd=TRUE)
title("Eligibility: all MSM")

par(mfg=c(1,2), mar=c(2, 4, 3, 1))
plot.new(); boxplot(t(red.matrix[table.order,,4,1]), xaxt="n",
 ylab="HPV16 prevalence reduction, penile", col=cols[modcols]); box()
text(x=c(0.5,5,12,18), y=0.425, c("SIS","systemic\nimmunity","local\nimmunity", "latency"), cex=0.9, xpd=TRUE)
title("40% boys at 12y")

par(mfg=c(1,3), mar=c(2, 4, 3, 1))
plot.new(); boxplot(t(red.matrix[table.order,,5,1]), xaxt="n",
 ylab="HPV16 prevalence reduction, penile", col=cols[modcols]); box()
text(x=c(0.5,5,12,18), y=0.535, c("SIS","systemic\nimmunity","local\nimmunity", "latency"), cex=0.9, xpd=TRUE)
...
plot.new(); boxplot(red.matrix[,,3,1], xaxt="n",,
 ylab="HPV16 prevalence reduction, penile", col=cols[modcols]); box()

par(mfg=c(2,2), mar=c(2, 4, 3, 1))
plot.new(); boxplot(red.matrix[,,4,1], xaxt="n",,
 ylab="HPV16 prevalence reduction, penile", col=cols[modcols]); box()
